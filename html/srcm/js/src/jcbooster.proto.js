export default function BoosterProto(Doc) {
		...
		let log = `
			<div class="box-content">
				...
				<div class="box-footer">
					<button class="__jcaction_setResolve btn btn-${type}" style="margin-right: 5px;">Confirm</button>
					<button class="__jcaction_setRejected btn btn-out-info">Cancle</button>	
				</div>
			</div>
		`;
		dv.innerHTML = log;
	...
	let __prompDialog = function(cf, cb) {
		let title 	= ( cf.title ) ? cf.title : "Promp",
			mode 	= ( cf.mode ) ? cf.mode : "text",
			type 	= ( cf.type ) ? cf.type : "secondary",
			ficon = ( cf.ficon ) ? cf.ficon : '',
			value = ( cf.value ) ? cf.value : '';

		let elemMode = {
			'text': `<input type="text" class="input-control __in_promp" value="${value}">`,
			'number': `<input type="number" class="input-control __in_promp" value="${value}">`,
			'long-text': `<textarea class="input-control __in_promp" rows="5">${value}</textarea>`
		};
		...
		let log = `
			<div class="box-content">
				...
				<div class="box-footer">
					<button class="__jcaction_promp_setResolve btn btn-${type}">Confirm</button>
					<button class="__jcaction_promp_setRejected btn btn-out-info" style="margin-left:5px;">Cancle</button>	
				</div>
			</div>
		`;
		dv.innerHTML = log;
	...
	jcApp.promp = async function(cf, cb) {
		__prompDialog(cf, elem => {
			new Promise( function(resolve, rejected) {
				elem.querySelector('.__jcaction_promp_setResolve').addEventListener('click', function() {
					resolve(elem.querySelector('.__in_promp').value);
				});
				elem.querySelector('.__jcaction_promp_setRejected').addEventListener('click', function() {
					rejected();
				});
			})
			.then( res => {
